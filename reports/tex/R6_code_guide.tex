...
\item \textbf{Tier 1d --- pair graph, certificates, weak charges} (separate
  \pf{results\_tier1d/} store, \rt{tier1d\_version 1d.1}; does \emph{not}
  recompute Tier 1a):\\
  \rt{python -m qca\_fragmentation.run\_rule --rule 22 --N 4-12 --bc pbc --tier 1d}\\
  \rt{python -m qca\_fragmentation.pair\_sweep --which coherent --n-min 4 --n-max 12}\\
  \rt{python -m qca\_fragmentation.quantum.weak\_charges --core}\\
  \rt{python -m qca\_fragmentation.scaling.pair\_scaling --bc pbc}\qquad(sandwich table)\\
  \rt{python -m qca\_fragmentation.scaling.pair\_figure --bc pbc}\qquad(coverage / sandwich / parity)
\item \textbf{C150 (rule 150) --- closed form, flip-graph frontier, spectra}
  (report R8; \pf{c150.py}, \pf{graph/flip\_graph.py},
  \pf{quantum/rule150\_spectra.py}, \pf{scaling/c150\_report.py}; the frontier is
  checkpointed to \rt{analytics/c150\_frontier.jsonl} and is rule-150 only):\\
  \rt{python -m qca\_fragmentation.c150 --verify}\\
  \rt{python -m qca\_fragmentation.c150 --frontier 21-32 --bc obc0}\\
  \rt{python -m qca\_fragmentation.scaling.c150\_report --quantum}\\
  \rt{python -m qca\_fragmentation.quantum.rule150\_levels --run}\qquad(level
  statistics)\\
  \rt{python -m qca\_fragmentation.scaling.c150\_report --tables --figures}
