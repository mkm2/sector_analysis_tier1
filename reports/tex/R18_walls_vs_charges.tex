...
ingredient. In detail: (i) the conserved densities are perfectly diagonal in the
computational basis --- $b_i = (1-Z_iZ_{i+1})/2$ --- and what fails is
\emph{range}, not basis: there is no single-site charge, so magnetisation is not
conserved, and separately the \emph{dipole} $P=\sum_i i\,b_i$ is not conserved
either, which is the real reason the charge/dipole mechanism does not apply here;
(ii) there is exactly one conserved domain-wall number per rule, and the two
families differ in which one --- $W_{108}/W_{201}$ conserve the \emph{staggered}
$S=\sum_i(-1)^i b_i$ while $W_{156}/W_{198}$ conserve the \emph{total}
$D=\sum_i b_i$, with the other broken in each case;
(iii) there are exactly two independent local charges, the sublattice-resolved
wall counts $|W_{\rm even}|$ and $|W_{\rm odd}|$, whose joint level sets number
$36$ at $N=14$ against $3329$ Krylov sectors --- short of the required
cardinality by an exponentially growing factor; (iv) each rule has exactly
\emph{one} minimal wall word ---
...
\subsection{These are computational-basis charges; what fails is range}

It is worth being careful here, because the loose phrase ``bond basis'' invites
a wrong reading. Everything in this report is diagonal in the computational
basis. The bond variable is
\[
b_i \;=\; \tfrac{1}{2}\bigl(1 - Z_i Z_{i+1}\bigr),
\]
a two-site diagonal operator, exactly as in the rule-150 domain-wall count of
R8; and the entire sector decomposition is a statement about basis states, so
nothing off-diagonal could enter. Bond variables are a change of \emph{variables},
not of basis, and Tier-2's \fn{bond=True/False} flag merely selects which local
variables the charge ansatz is built from.

What is actually true is a statement about \textbf{range}. Tier-2's detector
finds an empty null space at range $1$: there is no conserved quantity of the
form $\sum_i q(x_i)$, so magnetisation is not conserved --- immediate once one
notices that the gate flips a site outright. The conserved densities are range
$2$, and they are certified as such \emph{directly in the site basis}: each of
the four rules has a six-dimensional space of certified range-$2$ site-basis
charges, and $D$ (chiral pair) or $S$ (PXP-like pair) is among them, written as
a function of the two-site pattern $(x_i, x_{i+1})$.

\subsection{The dipole is the thing that is missing}

The charge/dipole mechanism needs a conserved density \emph{and} a conserved
first moment of it. The first exists here. The second does not:

\begin{proposition}
$P=\sum_i i\,b_i$, the dipole moment of the domain-wall density, is not
conserved for any of the four rules, at either boundary condition.
\end{proposition}

\noindent The reason is transparent from the moves. A single flip of $x_i$
toggles the two bonds $b_{i-1}$ and $b_i$; on the sectors where the wall number
is conserved they toggle oppositely, so a wall \emph{hops by one site}, leaving
$D$ fixed and shifting $P$ by $\pm1$. Forbidding exactly that hop is what a
dipole-conserving model does, and it is what these rules do not do. So the
correct statement is not that a diagonal charge is missing, but that the
conserved wall number comes without a conserved dipole.

\subsection{Exactly two independent charges, and they are wall counts}

The rank of the certified charge values on the full $2^N$ space is $3$ including
the constant, i.e.\ there are exactly \textbf{two} independent local conserved
quantities per rule --- the right number for the two-conservation-law story,
which is precisely why the story deserves testing rather than dismissal. They can
be named: the \textbf{sublattice-resolved wall counts}
\[
|W_{\rm even}| = \#\{i \in W : i \text{ even}\}, \qquad
|W_{\rm odd}|  = \#\{i \in W : i \text{ odd}\},
\]
for all four rules and both boundary conditions, which is why the detector needs
its parity split to see two of them. Every other charge in play is a function of
this pair: $D = 2(|W_{\rm even}|+|W_{\rm odd}|)$ for the chiral rules, and $S$ is
their difference up to sign. Their joint level sets reproduce Tier-2's full
certified charge set exactly --- $13, 18, 25$ at $N=8,10,12$ (pbc) for $W_{108}$
and $12, 17, 23$ for $W_{156}$ --- which is the cross-check that nothing
conserved has been left out of the comparison below.

Note already what this says: the two conservation laws are themselves wall data,
coarse-grained by sublattice.
...
\noindent The comparison uses \emph{both} independent charges, since testing the
hypothesis against one of them would understate the symmetry by a factor of $N$.
For $W_{108}$ the joint level sets of $(|W_{\rm even}|, |W_{\rm odd}|)$ number
$10, 15, 21, 28, 36$ over $N=6,8,10,12,14$ --- visibly quadratic --- while the
sectors number $37, 114, 351, 1081, 3329$. The ratio is therefore not merely
large but \emph{exponentially growing}: $3.7, 7.6, 16.7, 38.6, 92.5$. For
$W_{156}/W_{198}$ it runs $2.1, 3.7, 6.9, 13.5, 27.4$ over the same window.
...
number, and they are genuinely domain-wall charges, which is the right kind. But
their joint level sets number $O(N^2)$ --- $36$ of them at $N=14$ --- against
$3329$ Krylov sectors, and the ratio grows from $3.7$ to $92.5$ over
$N=6\ldots14$. They do not have the cardinality to be the explanation. Nor is
the missing ingredient a dipole: $\sum_i i\,b_i$ is not conserved, because the
walls hop.
