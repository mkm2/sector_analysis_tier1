...
prefactor, which supersedes the fitted $2.022$ of R2. We push the reduction to
$N=32$ --- $4.29\times10^{9}$ basis states, $21$~GB, $815$~s, the RAM ceiling ---
and the assumption-free engine as far as a one-day budget reaches
(see Table~\ref{tab:frontier} for which units have landed). The closed form is
exact at every $N$ computed, by either route.
...
project machine ($20$ cores, $31$~GB, jobs \fn{nice}d and run concurrently).
...
\item \textbf{Higher $N$ at the assumption-free frontier.} The streamed engine
  costs a factor $\approx3$ per site, so $N=23$ is a $\approx30$-hour unit and
  sits outside the one-day budget used here. The reduction frontier is instead
