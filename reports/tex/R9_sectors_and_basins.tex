...
\caption{\textbf{F7, the sector map in corner form} (\rt{obc0}). Main panel: the
...
\caption{\textbf{F8, the same for the monitored attractors} (\rt{obc0}), i.e.\
...
\caption{\textbf{F9, where the mass sits} (\rt{obc0}, monitored). Left: growth
...
\item \textbf{No dissipative rule is anywhere near it.} Not one of the $233$
  ...
  one.
\item \textbf{The V-free family is the more strongly contracting of the two},
  which is the opposite of what one might guess from the fact that it has no
  quantum gate. Its median mass base is exactly $1$ --- a \emph{bounded} recurrent
  set, a handful of frozen states at any $N$ --- against $\psi=1.46557$, the
  supergolden ratio, for the median V+reset rule. The Hadamard, by spreading a
  state over its sector, keeps exponentially more of the basis alive.
\end{itemize}

\noindent A caution on what F8 and F9 are. Both are statements about the
...
calls transient (R5's rule 22, R7's immortal transient). The sector map F7 is the
