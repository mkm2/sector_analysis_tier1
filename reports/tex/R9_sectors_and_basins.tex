...
must. Rule $156$ is the interesting case and is discussed in
...
\noindent \textbf{There are no violations.} All five remaining sub-$2$ rules
($74$, $88$, $104$, $173$, $233$) have a \emph{polynomial} sector count --- for
instance $W_{173}$ and $W_{88}$ both have $n_\wcc=3,3,4,4,4,5,5,5,6,6,6$, i.e.\
$\sim N/2$ --- so $a=1$ and the degenerate form of \eqref{eq:hyper} demands
$b\ge2$, which a finite-$N$ fit necessarily approaches from below
($1.90$--$1.96$ here). Two further rules, $W_{90}$ and $W_{165}$, have genuinely
irregular series ($n_\wcc=2,1,4,4,2,6,2,6,12,1,20$) that no growth law describes;
they are classified \emph{irregular} and omitted from the base plane rather than
placed at a fictitious coordinate.
...
sharper than the fit. At the $N\le11$ stage of this sweep the six rules $140$,
$156$, $196$, $198$, $206$, $220$ all had an \emph{exact} $a=\varphi$ with a
$D_{\max}$ series the BIC still called polynomial, so $ab=\varphi<2$; the theorem
already forced $b\ge2/\varphi=1.2361$ while the fitted band topped out at
$1.2369$, pinning the exponent to a window of width $10^{-3}$ before the data
could resolve it. Carrying the sweep to $N=16$ confirmed the prediction: rule
$156$ now reads $b=1.2554$ and $ab=2.0313$, \textbf{above} the curve, satisfying
V3. (The expected $b=4^{1/5}=1.3195$ remains a little above the measured value;
resolving that gap needs larger $N$ and is listed as an open item.)
...
\item \textbf{Higher $N$ for the $\varphi$ family.} $N\le16$ gives rule $156$
  $b=1.2554$ against the expected $4^{1/5}=1.3195$; the theorem floor is
  $2/\varphi=1.2361$. Settling whether the true value is $4^{1/5}$ needs larger
  $N$ than this sweep reached.
