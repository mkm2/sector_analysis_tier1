...
\noindent \textbf{There are no violations, and since R16 there is nothing left
below the curve either.} Seven rules ($6$, $14$, $20$, $74$, $84$, $88$, $229$)
used to appear here with $ab=1.919$--$1.937$. Their sector counts are exact
quasi-linear staircases --- $\lceil N/2\rceil+1$ for $6,14,20,84$ and
$\lfloor(N+4)/3\rfloor$ for $74,88,229$, verified at every $N\le22$ --- so $a=1$,
and the degenerate form of \eqref{eq:hyper} together with $D_{\max}\le2^N$
squeezes $b$ to exactly $2$. The pair $(1,1.92)$ was therefore not a violated
theorem but an internally inconsistent descriptor, and R16 replaces it with the
derived $(1,2)$ plus a refitted prefactor power. The extended sweep to $N=22$
supports the correction independently: $W_{229}$'s largest sector \emph{stops
decaying} relative to $2^N$ on the odd branch, giving a local growth rate of
$2.0052$ over $N=15\ldots21$, which no base below $2$ can produce. Two further
rules, $W_{90}$ and
...
assertion A2 made visible. \textbf{The three families are drawn with a
horizontal offset of $\pm0.019$} (V+reset right, V-free left, unitary
unshifted): without it the $144$ V+reset rules at $(1,2)$ sit exactly under $33$
V-free ones and one family is invisible. Positions within a family are therefore
exact relative to each other, but an absolute base read off the axis carries that
offset --- take exact values from the tables, not from the figure.
\textbf{At the top-left corner that offset is not merely cosmetic}: $b=2/a$ rises
as $a$ falls, so the $40$ V-free rules that sit exactly \emph{on} the curve at
$(1,2)$ are drawn at $a=0.981$, where the curve is $2.0387$, and therefore appear
\emph{under} it. They are the largest marker in the panel and they are not
violations; see R16.}
\label{fig:f1}
\end{figure}
