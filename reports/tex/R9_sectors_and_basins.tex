...
crowded cells, without which the distribution is invisible: $73\%$ of the sector
map lies in one cell. Colour is by family, with the quantum rules (those carrying
a Hadamard) drawn forward and the V-free rules held back as a baseline. The nine
...
The left panel of Fig.~\ref{fig:f1} has a blunt message: \textbf{$186$ of $256$
rules sit at exactly $(a,b)=(1,2)$} --- one sector, of size $2^N$. Broken down by
family:
...
\noindent So $90\%$ of the rules that combine a Hadamard with a reset have
...
has a corollary for how this tier should be used. For those $144$ rules the
...
\noindent The unitary median is negative because those rules sit \emph{on or
above} the curve in both planes --- for a unitary rule the two planes are the same
plane. The V-free median of $+0.909$ says $a_\att b_\att\approx1$: their
attractors are $O(1)$ fixed points and essentially the entire space is transient.
The V+reset family sits between the two.
...
sector plane all $144$ of them collapse onto $(1,2)$.}
\label{fig:f2}
\end{figure}
...
attractors are $O(1)$ fixed points --- while the unitary rules sit at $0$ or
below.}
\label{fig:f3}
\end{figure}
...
\begin{itemize}
\item \textbf{Eight parents already sit at $(1,2)$} --- one sector holding the
  whole space --- so there is nothing for a reset to destroy, and all $40$ of
  their children stay exactly where the parent is.
\item \textbf{Six parents have genuine sector structure}, and they hold $120$
  children between them. \emph{Not one of the $120$ keeps its parent's
  position.} $104$ collapse all the way to $(1,2)$; the remaining $16$ move to
  some intermediate point but never stay.
\end{itemize}
...
structure, without exception over the $120$ cases available}. It also explains
the pile-up of \S\ref{sec:pileup} arithmetically --- the $144$ V+reset rules at
$(1,2)$ are exactly the $104$ that collapsed plus the $40$ that were never
anywhere else.
...
drawn. These coordinates are exact for both experiments.}
\label{fig:f6}
\end{figure}
...
\item \textbf{Which V+reset rules escape the collapse} (\S\ref{sec:pileup}).
  Only $16$ of $160$ keep any sector structure; a grammar should predict that
