...
attractor. That is worth flagging against F9's caption, which records the shared
basin as identically zero for the rules shown there; it is zero for those rules,
not in general, and the sixteen single-attractor survivors here are zero too. A
shared basin needs two attractors in one sector to be possible at all, so the two
columns switch on together.

\noindent The renderings are not embedded --- $66$ pictures at this size would
dominate the document --- and are not tracked either: they are $112$\,MB of
\fn{.dot} and \fn{.png} that no reader needs in the history. They live in
\fn{figures/sector\_graphs/} as
\fn{N11\_\{rule\}\_V4\_obc0\_\{wcc,scc,joint\}.\{dot,png\}}, and the placement
RNG is seeded per rule, so
\begin{center}
\fn{python -m qca\_fragmentation.viz.dot\_sectors -{}-N 11 -{}-bc obc0}
\end{center}
reproduces all $66$ byte-for-byte in about ten minutes. Pass
\fn{-{}-variant V3} for the core-periphery layout instead of the archipelago
one, and \fn{-{}-rules 29,157} to draw a chosen pair.
