...
both boundary conditions; Tier-2 R-T21~\S2 (Prop.~2) reports the same over
$N=3\ldots13$ from an independent implementation.
\end{theorem}
...
longest equal to $3\ell-7$ (verified here to $\ell=16$, and by Tier-2
R-T21~\S5 to $\ell=26$, where it is written $3\ell-13$ in that report's
collar-counting indexing).
...
($v=5$ is $(5)$ for the ascent pair and $(2,3)$ for the symmetric one). Tier-2 R-T21~\S2 indexes by the geometric gap and therefore
needs a left/interior/right class; the two are the same table, and its
paragraph ``The dictionary between the two indexings'' gives the translation
from that side. Their
...
wall-free label is never a segment'', which is false on the chain. Tier-2 R-T21~\S4
...
$N=18$; the trap below is that report's \emph{Implementation warning} in
its~\S6.
...
laws are not separated. They have since been separated, by Tier-2 R-T21~\S6, and the
answer is $\Theta(\ln N)$.

\begin{proposition}[Tier-2 R-T21~\S6; anchor reproduced here]
\label{prop:logn}
...
The complementary transfer-matrix treatment is Tier-2 report R-T21 (5\,pp):
its \S2 carries the corner-class indexing and the dictionary between the two
parameterisations, \S4 the wall-free label, \S5 the segment spectrum to
$\ell=26$ and the ring wall-free spectra, and \S6 the $\Theta(\ln N)$ law with
the $1/\ln2$ mechanism.
