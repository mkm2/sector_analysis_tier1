...
$C$ is indexed here by the segment's \emph{number of states}, which makes the
corner class redundant --- keyed that way there are zero conflicts for all four
rules at both boundaries. Tier-2 R-T21 indexes by the geometric gap and therefore
...
boundary conditions. (The ring needing the budget spent \emph{exactly} rather
...
\end{tabular}
\end{center}
The first ratio rises and the second falls, so the truth lies between
$\Theta(\ln N)$ and $\Theta(\ln^2 N)$ and \textbf{these sizes do not separate
them}. We state that as an open question rather than fit an exponent to five
points. The mechanism argues for the lower end: with $\Theta(N)$ segments and an
exponential tail on segment length the longest cycle in a typical label is
$O(\ln N)$, and all cycle lengths are $O(\ell)$ (Tier-2 R-T21, to $\ell=26$), so
$\ln\mathrm{lcm}$ should be a sum of $\ln p$ over primes realised below an
$O(\ln N)$ ceiling --- which gives $\Theta(\ln N)$ with a slowly growing
correction. Settling it needs $N\sim40$--$60$ on the typical value.
