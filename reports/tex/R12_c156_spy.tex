...
for rules $156$, $157$, $109$, $150$ and $158$, chosen to span both axes:
...
SCCs, i.e.\ the monitored attractors --- first, sources last, larger first among
those free to move. An edge then runs from a higher index to a lower one, so
every nonzero sits on or \emph{above} the block diagonal. The attractors occupy
the top-left corner and everything drains up and to the left.
\end{definition}
...
\begin{itemize}
\item \textbf{Nothing lies below the block diagonal, for any of the five.} The
  Frobenius form is genuinely triangular, computed and not assumed.
