...
its unitary parent --- identically, to zero, for $N = 4,\dots,11$.
\end{abstract}
...
the protected space directly, since Prop.~\ref{prop:unitary} makes $D$ the
number of recurrent basis states. For $W_{73}$ and $W_{109}$, obc0, $N$ up to
$18$:

\begin{center}
\begin{tabular}{lll}
\toprule
quantity & growth & identification \\
\midrule
$n_{\mathrm{rec}}$, the number of enclosures & $\psi = 1.4656$ &
  supergolden, A000930 \\
$d^{\mathrm{rec}}_{\max}$, the largest enclosure & $\varphi = 1.6180$ &
  Fibonacci, the blockade space \\
$D$, the whole protected space & $1.8393$ &
  \textbf{tribonacci constant} \\
\bottomrule
\end{tabular}
\end{center}

\noindent $D$ for $W_{73}$ runs $13, 24, 44, 81, 149, 274, 504, 927, 1705,
3136, 5768, \dots$ --- each term the sum of the previous three, ratio settled at
$1.8393$ by $N=9$; $W_{109}$ gives $9, 17, 31, 57, 105, 193, 355, \dots$, the
same recurrence and the same constant. So the total decoherence-free dimension
grows \emph{faster} than any single enclosure: $\log_2 1.8393 = 0.879$ qubits
per site protected in total, against $\log_2\varphi = 0.694$ inside the largest
blockade sector. The difference is exactly the cross-enclosure coherence that
Prop.~\ref{prop:unitary} says survives, and that a per-attractor account would
miss.

The other column worth having: $n_{\mathrm{wcc}} = n_{\mathrm{rec}}$ at
\emph{every} $N$ from $4$ to $18$, for both rules. These two are fragmented in
both senses with no gap at any size we can reach --- which is what a paper built
on ``fragmented in both senses, and non-unitary'' needs, and which
...
a reason, and it is one line of gate arithmetic.
...
the constrained sector. If the paper introduces Floquet-PHP and Floquet-PHQ as
its two unitary models, then Fig.~3's dissipative counterparts are the same two
words with one letter changed, which is about as compact as a third figure can
get. That is also why an SCC census suffices there, and why deferring the
WCC/SCC distinction to a companion paper is defensible: for seven of the eight,
$n_{\mathrm{wcc}} = n_{\mathrm{rec}}$ at every size we can check coherently.
