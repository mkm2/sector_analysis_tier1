...
\item \textbf{The dissipative $\chi(t)$ figure: dropped, and it should stay
dropped.} R23 reports $\chi_{\mathrm{sat}}$ saturating \emph{in $N$} for the six
and growing exponentially for $W_{73}/W_{109}$. A headline about linear growth
\emph{in time} is a different axis, and the dissipative rules were never
measured on it. We record here why that measurement was abandoned rather than
scheduled, because a later reader will otherwise re-propose it.

The structural objection is that there is nothing for the figure to be.
Proposition~\ref{prop:unitary} says there is no dissipation \emph{inside} an
enclosure, and Proposition~\ref{prop:allparents} says the child and its coherent
parent agree there exactly. So $\chi(t)$ measured inside the attractor
\emph{is} the unitary parent's curve --- the same circuit, one letter apart in
the gate word --- while $\chi(t)$ measured across the transient is a statement
about relaxation, not about fragmentation. Neither is ``the dissipative model
inherits linear $\chi(t)$ growth''. A practical obstacle points the same way:
the superoperator route OOMs for $W_{73}$ at $N=7$, $t=5$ (27\,GB, the zip-up
materialising an untruncated bond at $16\chi \approx 12000$), so those two would
need a contraction that never forms that bond. An earlier full-space measurement
on the project's side reached the same verdict independently.

The claim the physics does support needs no third measurement:
\emph{the dissipation is transient-only, so the dissipative rules inherit
fragmentation, and the object carrying the growth is the unitary parent the
paper already introduces.}
\end{enumerate}
