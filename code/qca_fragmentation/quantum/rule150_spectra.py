...
at site i acts only on the pair of bonds (i-1, i); it is the identity when both
or neither carry a wall, and on the one-wall subspace it is

      1/sqrt2 * [[(-1)^c, 1], [1, -(-1)^c]],    c = x_{i-1} = parity of the
                                                walls strictly left of bond i-1,
...
#
# Eigenvalue clustering.  The degeneracies of U_w are EXACT (repeated to machine
# zero) and well separated from the rest of the spectrum: over every sector
# computed here the smallest nonzero phase gap is >= 4.6e-3 while the degenerate
# gaps are < 1e-13.  A cluster tolerance of 1e-9 therefore separates true
# multiplicities from distinct eigenvalues with a margin of six orders of
